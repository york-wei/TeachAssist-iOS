...
struct TeachAssistResponse {
    ...
    init(_ innerHTML: Any?) throws {
        ...
        if let body = try doc.getElementsByClass("green_border_message box").last()?.getElementsByTag("tbody").first() {
            ...
            for tr in body.children() { //For each course on the reports page
                ...
                if let body2 = try tr.getElementsByAttribute("bgcolor").first(){
                    ...
                    let course = Course(code: code, name: name, period: period, room: room, mark: mark, link: link, marks: arr, markStruct: holderMarkStruct, prevAverage: -2, updated: false)
